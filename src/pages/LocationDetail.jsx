...
function LocationDetail() {
  ...
  const relateItem = locationData.filter((item) => item.relate === locations.relate && item.id !== id.id);
    ...
    <div className="container detail-container">
      ...
      <div className="row row-1">
        <div className="col-sm-12">
          <h2 className="title-detail-loc">{locations.title} | {locations.date}</h2>
          <Slider asNavFor={nav2} ref={(slider1) => setNav1(slider1)} className="p-2" {...settings}>
            {locations.listURL.map((item) => (
              <SliderLocationCard item={item} index="1">

              </SliderLocationCard>
            ))}
          </Slider>

          <Slider asNavFor={nav1} ref={(slider2) => setNav2(slider2)} className="p-2" {...settings2}>
            {locations.listURL.map((item, index) => (
              <SliderLocationCard item={item} index="2" current={index}>
