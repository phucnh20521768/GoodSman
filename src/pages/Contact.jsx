...
function Contact() {
    ...
    var submitBtn = function (e) {
        if (isVerified) {
            ...
            axios.post('https://sheet.best/api/sheets/f5c1792b-3031-4731-bd6e-08574fceba2e', data);
            setFirstName('');
            setLastName('');
            setEmail('');
            setComment('');
                ...
                <form onSubmit={submitBtn} className='contact-form'>
                    <div className='row'>
                        <div className='col-md-4 mb-2 p-2'>
                            <input id='first-name' type="text" class="form-control contact-input border-dark border-0 border-bottom" maxLength='255' required placeholder="Họ"
                            onChange={(e) => setFirstName(e.target.value)} value={firstName}></input>
                        </div>
                        <div className='col-md-4 mb-2 p-2'>
                            <input id='last-name' type="text" class="form-control contact-input border-dark border-0 border-bottom" maxLength='255' required placeholder="Tên"
                            onChange={(e) => setLastName(e.target.value)} value={lastName}></input>
                        </div>
                        <div className='col-md-4 mb-2 p-2'>
                            <input id='email' type="email" class="form-control contact-input border-dark border-0 border-bottom" maxLength='255' required placeholder="Email"
                            onChange={(e) => setEmail(e.target.value)} value={email}></input>
                        </div>
                        <div className='p-2'>
                            <textarea id='comment' rows="1" class="form-control contact-input my-2 border-dark border-0 border-bottom resize-vertical" number='3' required placeholder="Ghi chú"
                    ...
                    />
                    <span className='error' id='error'>Vui lòng xác minh bạn không phải là robot</span>
                    <div className='div-submit-contact'>
                        <input onClick={checkCaptcha} type='submit' className='btn btn-primary btn-lg contact-submit' value='Gửi'></input>
                    </div>
                </form>
            </Container>
        </Container>
